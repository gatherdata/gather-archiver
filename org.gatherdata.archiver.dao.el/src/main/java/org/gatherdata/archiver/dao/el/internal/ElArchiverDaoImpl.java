...
public class ElArchiverDaoImpl implements ArchiverDao {
    ...
    public Serializable save(Serializable contentToSave) {
        // ABKTODO: what's the archiver do with content lacking an envelope? 
        // should it create a generic envelope, then save it all? probably
        throw new NotImplementedException();
    }
    ...
    public Envelope getProxiedEnvelope(URI identifiedByUid,
        ...
        EnvelopeDTO proxiedEnvelope = getEnvelopeDTO(identifiedByUid);
        proxiedEnvelope.setContents(usingAlternateContentDoa.getContent(identifiedByUid));
        return proxiedEnvelope;
    }
    ...
    public Envelope saveProxy(Envelope envelopeToSave,
            ContentStorageDao<? extends Serializable> usingAlternateContentDoa) {
        EnvelopeDTO envDTO = new EnvelopeDTO(envelopeToSave);
        Serializable contents = envDTO.getContents();
        envDTO.setContents(null);
        DatabaseSession dbSession = createDbSession();
        dbSession.login();
        EnvelopeDTO savedEnvelope = (EnvelopeDTO)dbSession.writeObject(envDTO);
        dbSession.logout();
        usingAlternateContentDoa.saveAdapted(contents);
        return savedEnvelope;
